...
async function ensureUiComponents(pkgPath, dorequire = false) {
    ...
    let _iconImportStatements = [];
    let _iconModelDeclarations = [];
    svgs.forEach(svg => {
        ...
        if (dorequire) {
            ...
            _iconModelDeclarations.push(`{ name: '${name}', svg: require('${svgpath}').default }`);
        }
        else {
    ...
    const iconImportsContents = utils.fromTemplate(HEADER_TEMPLATE + ICON_IMPORTS_TEMPLATE, { funcName, iconImportStatements, iconModelDeclarations });
